...
#property description "XAUUSD M1 scalper with M5 trend, risk protection, live stats and basket management"
...
{
   MqlDateTime tm; TimeToStruct(TimeCurrent(),tm);
   int shift=(tm.day_of_week==0?6:tm.day_of_week-1);
   tm.day-=shift; tm.hour=0; tm.min=0; tm.sec=0;
   return StructToTime(tm);
}
...
double ClosedPLSince(datetime fromTime)
{
...
double GrossProfitSince(datetime fromTime)
{
   if(!HistorySelect(fromTime,TimeCurrent())) return 0.0;
   double total=0.0;
   for(int i=0;i<HistoryDealsTotal();i++)
...
double GrossLossSince(datetime fromTime)
{
   if(!HistorySelect(fromTime,TimeCurrent())) return 0.0;
   double total=0.0;
   for(int i=0;i<HistoryDealsTotal();i++)
...
int ClosedCountSince(datetime fromTime)
{
...
int WinsSince(datetime fromTime)
{
   if(!HistorySelect(fromTime,TimeCurrent())) return 0;
   int n=0;
   for(int i=0;i<HistoryDealsTotal();i++)
...
bool DailyLossBlocked()
{
   ...
   double startBalance=AccountInfoDouble(ACCOUNT_BALANCE)-closed;
   if(startBalance<=0) return false;
   return (closed+BasketProfit()) <= -(startBalance*DailyLossLimitPercent/100.0);
}
...
int Signal()
{
   ...
   double close1=iClose(sym,EntryTF,1);
   double trendClose=iClose(sym,TrendTF,1);
   if(e21==EMPTY_VALUE||e34==EMPTY_VALUE||e55==EMPTY_VALUE||e200==EMPTY_VALUE||rsi==EMPTY_VALUE) return 0;
   bool up=trendClose>e200 && e21>e34 && e34>e55 && close1>e21;
   bool dn=trendClose<e200 && e21<e34 && e34<e55 && close1<e21;
   bool stochUp=(k2<=d2 && k1>d1 && k1<45);
   bool stochDn=(k2>=d2 && k1<d1 && k1>55);
   double poc,vah,val; bool vp=VolumeProfile(poc,vah,val);
   double bid=SymbolInfoDouble(sym,SYMBOL_BID), ask=SymbolInfoDouble(sym,SYMBOL_ASK);
   if(AllowLong && up && rsi<=RSI_BuyMax && stochUp && (!vp || ask>=val)) return 1;
   if(AllowShort && dn && rsi>=RSI_SellMin && stochDn && (!vp || bid<=vah)) return -1;
   return 0;
}

double LastEntryPrice(int dir)
{
   double price=0; datetime latest=0;
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(ot>=latest){latest=ot; price=PositionGetDouble(POSITION_PRICE_OPEN);}
   }
   return price;
}
double NextOrderPrice(int dir)
{
   ...
   if(last<=0) last=WeightedOpenPrice();
   if(last<=0) return 0;
   double step=atr*Grid_ATR_Mult;
   return (dir>0?last-step:last+step);
}
...
void Dashboard()
{
   ...
   double today=ClosedPLSince(StartOfDay()), week=ClosedPLSince(StartOfWeek()), month=ClosedPLSince(StartOfMonth()), total=ClosedPLSince(0);
   double gp=GrossProfitSince(0), gl=GrossLossSince(0), pf=(gl>0?gp/gl:0);
   int closed=ClosedCountSince(0), wins=WinsSince(0);
   double wr=(closed>0?100.0*wins/closed:0);
   double poc=0,vah=0,val=0; VolumeProfile(poc,vah,val);
   double next=NextOrderPrice(BasketDirection()), btp=BasketTPPrice(BasketDirection());
   ...
   string txt=
      ...
      "v1.01 | "+sym+" | M1 scalp / M5 trend\n"+
      "Balance  "+DoubleToString(bal,2)+" USD\n"+
      "Equity   "+DoubleToString(eq,2)+" USD\n"+
      "Open P/L "+DoubleToString(BasketProfit(),2)+" USD\n"+
      "Today    "+DoubleToString(today,2)+" USD\n"+
      "Week     "+DoubleToString(week,2)+" USD\n"+
      "Month    "+DoubleToString(month,2)+" USD\n"+
      "Total    "+DoubleToString(total,2)+" USD\n"+
      "Closed   "+IntegerToString(closed)+" | Win "+DoubleToString(wr,1)+"% | PF "+DoubleToString(pf,2)+"\n"+
      "Position "+PosText()+"\n"+
      "Orders   "+IntegerToString(CountPositions())+"/"+IntegerToString(MaxPositions)+"\n"+
      "Next lot "+DoubleToString(NormalizeLot(FixedLot*MathPow(GridMultiplier,CountPositions())),2)+"\n"+
      "Next entry "+(next>0?DoubleToString(next,DigitsSym()):"--")+"\n"+
      "Basket TP "+(btp>0?DoubleToString(btp,DigitsSym()):"--")+"\n"+
      "POC "+(poc>0?DoubleToString(poc,DigitsSym()):"--")+" | VAH "+(vah>0?DoubleToString(vah,DigitsSym()):"--")+" | VAL "+(val>0?DoubleToString(val,DigitsSym()):"--")+"\n"+
...
void OnTick()
{
   ...
   bool blocked=DailyLossBlocked();

   if(CountPositions()==0)
   {
      if(!blocked && !SuddenMoveBlocked() && spread<=MaxSpreadPoints) 
      {
         int sig=Signal();
         if(sig!=0) OpenTrade(sig);
      }
   }
   else if(CountPositions()<MaxPositions && !blocked && spread<=MaxSpreadPoints)
   {
      int dir=BasketDirection();
      if(dir!=0)
      {
         ...
         double bid=SymbolInfoDouble(sym,SYMBOL_BID), ask=SymbolInfoDouble(sym,SYMBOL_ASK);
         if((dir>0 && bid<=next) || (dir<0 && ask>=next)) OpenTrade(dir);
      }
   }
   Dashboard();
}
